...
進入「銷售分析」，選擇一個已啟用的多門店帳號、勾選門店，再查詢月份或自訂日期範圍。月份比較會在一次操作中取得本期、上期、前期、去年同期與去年下月；每個「期間 × 門店」工作共用所選帳號，並依「設定」中的並行數執行。選擇尚未結束的本月時，本期、上期、前期與去年同期會截止於相同日號（短月份以月底為限）；去年下月使用完整月份。
...
分類頁會同時顯示本期／去年同期／去年下月的銷售額排行，以及本期／上期／前期的銷量排行。「匯出門店 PDF」會一次選擇資料夾，再為每間成功查詢的門店分別產生一份橫向 PDF；同名檔案會自動加後綴，不會覆寫舊報告。
...
內建 solver 對清楚字形保留快速的雙模型共識路徑；遇到邊界案例或封閉環易混字（`0`/`8` 與 `c`/`e`/`3`/`9`）時，會改用多組色差重新擷取，再交由兩套形狀模型加上孔洞與開口檢查投票。信心不足的圖片不會送出；系統會取得新的 challenge，或交給下一個已設定的 solver。
