...
export function buildSalesAnalysisAIMarkdown(input: SalesReportAIInput): string {
  ...
  const lines: string[] = [
    ...
    `purpose: auto-run-sales-briefing`,
...
function aiCopy(locale: Locale) {
  if (locale === 'en') {
    return {
      ...
      autoRun: 'This file is the complete task and the data. As soon as you receive it, write the report. The user will not add another prompt.',
      taskTitle: 'Write this report now',
      task1: 'Using only the period-totals table, say how this period did versus the previous period, two periods ago, and the same period last year. Skip a comparison if that period is not in the file.',
      task2: 'From the category table, name the listed categories that rose or fell the most versus the previous period, and by how much. If two-periods-ago figures exist, mention those too.',
      task3: 'Give at most 5 next-month actions. Prefer the last-year-next-month section when it exists; otherwise use the category table.',
      task4: 'Keep it short. Every claim must cite one number already in this file.',
      instructions: 'Rules (must follow)',
      ruleSource: 'Use only the numbers in this file. If a figure is not here, write that it is not in the file.',
  ...
  return {
    ...
    audience: '任何大型語言模型',
    allStores: '全部門店',
    autoRun: '這份檔就是完整任務與資料。收到後立刻寫出報告，使用者不會再另外下指令。',
    taskTitle: '立刻寫出這份報告',
    task1: '只用「各期間總數」裡的數字，說明本期相對上期、前期、去年同期。檔裡沒有的期間就跳過。',
    task2: '分類表裡哪些分類較上期升或跌最多？寫出幅度。有前期就一併寫。',
    task3: '下個月最多給 5 個行動。有「去年下月關注」就先用那一節；沒有再用分類表。',
    task4: '短句。每一個判斷都要引用這份檔裡已有的一個數字。',
    instructions: '寫的時候必須遵守',
