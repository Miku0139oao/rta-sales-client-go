...
describe('cross-format export identities', () => {
  ...
  it('compares screen/Excel/PDF/AI actuals to an independent numeric table', async () => {
    ...
    expect(categories?.rows[0]?.cells[2]).toBe(7800 / currentTotal);
    expect(categories?.rows[0]?.cells[3]).toBe(3900);
    expect(categories?.rows[0]?.cells[4]).toBe(3900 / previousTotal);
    expect(categories?.rows[0]?.cells[7]).toBe(6240);
    expect(categories?.rows[0]?.cells[8]).toBe(6240 / yearAgoTotal);
