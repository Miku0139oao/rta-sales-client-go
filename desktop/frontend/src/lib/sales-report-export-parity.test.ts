...
describe('cross-format export identities', () => {
  ...
  it('compares screen/Excel/PDF/AI actuals to an independent numeric table', async () => {
    ...
    const currentTotal = categorySums(currentItems).reduce((sum, group) => sum + group.amount, 0);
    const previousTotal = previousCats.reduce((sum, group) => sum + group.amount, 0);
    const yearAgoTotal = yearAgoCats.reduce((sum, group) => sum + group.amount, 0);
    ...
    expect(categories?.rows[0]?.exportCells?.[1]).toBe(7800);
    expect(categories?.rows[0]?.exportCells?.[2]).toBe(7800 / currentTotal);
    expect(categories?.rows[0]?.exportCells?.[3]).toBe(3900);
    expect(categories?.rows[0]?.exportCells?.[4]).toBe(3900 / previousTotal);
    expect(categories?.rows[0]?.exportCells?.[7]).toBe(6240);
    expect(categories?.rows[0]?.exportCells?.[8]).toBe(6240 / yearAgoTotal);
