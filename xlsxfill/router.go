...
// share one authenticated session and therefore must not be queried at once;
// when empty it is derived from Provider.
...
// explicit authenticated-session lanes. It copies every route and never
// serializes the supplied profile/lane metadata.
